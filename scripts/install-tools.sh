...
JAVA_ARCHIVE="OpenJDK21U-jdk_x64_linux_hotspot_${JAVA_VERSION}.tar.gz"
JAVA_URL="https://github.com/adoptium/temurin21-binaries/releases/download/jdk-21.0.10%2B7/${JAVA_ARCHIVE}"
JAVA_TARGET_DIR="$TOOLS_DIR/jdk-21.0.10+7"

CC_JAVA_VERSION="11.0.27_6"
CC_JAVA_ARCHIVE="OpenJDK11U-jdk_x64_linux_hotspot_${CC_JAVA_VERSION}.tar.gz"
CC_JAVA_URL="https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.27%2B6/${CC_JAVA_ARCHIVE}"
CC_JAVA_TARGET_DIR="$TOOLS_DIR/jdk-11.0.27+6"
...
download_to "$NODE_URL" "$TOOLS_DIR/$NODE_ARCHIVE" "$NODE_TARGET_DIR"
install_tar_xz "$NODE_TARGET_DIR" "$TOOLS_DIR/$NODE_ARCHIVE"

download_to "$JAVA_URL" "$TOOLS_DIR/$JAVA_ARCHIVE" "$JAVA_TARGET_DIR"
install_tar_gz "$JAVA_TARGET_DIR" "$TOOLS_DIR/$JAVA_ARCHIVE"

download_to "$CC_JAVA_URL" "$TOOLS_DIR/$CC_JAVA_ARCHIVE" "$CC_JAVA_TARGET_DIR"
install_tar_gz "$CC_JAVA_TARGET_DIR" "$TOOLS_DIR/$CC_JAVA_ARCHIVE"

download_to "$KAFKA_URL" "$TOOLS_DIR/$KAFKA_ARCHIVE" "$KAFKA_TARGET_DIR"
install_tar_gz "$KAFKA_TARGET_DIR" "$TOOLS_DIR/$KAFKA_ARCHIVE"

download_to "$NATS_URL" "$TOOLS_DIR/$NATS_ARCHIVE" "$NATS_TARGET_DIR"
install_tar_gz "$NATS_TARGET_DIR" "$TOOLS_DIR/$NATS_ARCHIVE"

download_to "$MAVEN_URL" "$TOOLS_DIR/$MAVEN_ARCHIVE" "$MAVEN_TARGET_DIR"
install_tar_gz "$MAVEN_TARGET_DIR" "$TOOLS_DIR/$MAVEN_ARCHIVE"

if [ ! -e "$TOOLS_DIR/jdk-21.0.10+7-jre" ]; then
  ln -s "jdk-21.0.10+7" "$TOOLS_DIR/jdk-21.0.10+7-jre"
fi

echo "Installed workspace tools under $TOOLS_DIR"
echo "Node:  $NODE_TARGET_DIR"
echo "Java:  $JAVA_TARGET_DIR"
echo "CC JDK: $CC_JAVA_TARGET_DIR"
