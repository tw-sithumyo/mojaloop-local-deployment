...
if [ "$WALLET_STACK" != "pivotal" ]; then
  WALLET1_CURRENCY="$CURRENCY"
  WALLET1_INITIAL_NET_DEBIT_CAP="$INITIAL_NET_DEBIT_CAP"
  WALLET1_INITIAL_FUNDS="$INITIAL_FUNDS"
fi

API_BASE="http://127.0.0.1:3001"
SOURCE_HEADER="wallet1-setup"
...
api_post() {
  ...
  status="$(
    curl -sS \
      -o /tmp/wallet1-api-response.json \
      -w '%{http_code}' \
      -X POST \
  ...
  cat /tmp/wallet1-api-response.json >&2
  echo >&2
...
api_get_status() {
  local path="$1"
  curl -sS -o /tmp/wallet1-api-response.json -w '%{http_code}' \
    -H "FSPIOP-Source: $SOURCE_HEADER" \
    "$API_BASE$path"
}
...
ensure_funds() {
  ...
  api_post "/participants/$PARTICIPANT_NAME/accounts/$account_id" "{
    ...
    \"externalReference\": \"wallet1-bootstrap\",
